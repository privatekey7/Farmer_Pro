...
class TwitterCheckerConfigWidget(QWidget):
    """Config panel for Twitter Checker module."""
    ...
    def __init__(self, parent=None) -> None:
        ...
        for s in ("ok", "invalid", "suspended", "locked"):
    ...
    def _on_export(self) -> None:
        if not self._export_results:
            return
        from app.storage.twitter_exporter import TwitterExporter, TwitterExportConfig

        status_filter = self._status_combo.currentData()
        fmt = self._fmt_combo.currentData()
        config = TwitterExportConfig(status_filter=status_filter)

        extensions = {
            "csv":  (tr("save_csv"),  "twitter_results.csv",  "CSV (*.csv)"),
            "json": (tr("save_json"), "twitter_results.json", "JSON (*.json)"),
            "xlsx": (tr("save_xlsx"), "twitter_results.xlsx", "Excel (*.xlsx)"),
        }
        title, default_name, file_filter = extensions[fmt]
        path, _ = QFileDialog.getSaveFileName(self, title, default_name, file_filter)
        if not path:
            return
        TwitterExporter().export(self._export_results, config, path, fmt)
