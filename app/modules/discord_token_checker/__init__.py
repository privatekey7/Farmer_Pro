...
def _check_token_sync(
    ...
    """Module-level sync function for run_in_executor. Checks one Discord token."""
    ...
    for _ in range(RETRY_ATTEMPTS):
        if stop_event.is_set():
            return Result(
                ...
                data={"username": None, "user_id": None, "email": None,
                      "has_phone": False, "account_status": "error"},
            )
        ...
        try:
            r = DiscordClient(proxy.to_url()).check_token(token)

            if r.status == DiscordTokenStatus.OK:
                return Result(
                    item=token,
            ...
            if r.status in (DiscordTokenStatus.INVALID, DiscordTokenStatus.DISABLED):
...
class DiscordTokenCheckerModule(BaseModule):
    ...
    def __init__(self) -> None:
        ...
        self._signals = _DiscordSignals()
        self._results: list[Result] = []
        self._widget = DiscordCheckerConfigWidget()
